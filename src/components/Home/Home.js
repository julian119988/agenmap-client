...
export default function Home() {
  ...
  useEffect(() => {
    filterRepeatedStays(); // eslint-disable-next-line
  }, [stays]);

  useEffect(() => {
    filterSearchResults(); // eslint-disable-next-line
  }, [location]);
  ...
  function filterSearchResults() {
    const tempArray = [];
    filteredStays.forEach((item) => {
      if (
        !(item.city.toLowerCase().search(location.toLocaleLowerCase()) === -1)
      ) {
        tempArray.push(item);
      } else if (
  ...
  function setLocationOnClick(event) {
    if (event.target.nodeName === "LI") {
      ...
      setLocation(
        event.target.ownerSVGElement.parentElement.nextSibling.textContent
      );
          ...
          {searchResults[0] ? (
            <LocationList>
              {searchResults.map((item, index) => (
                <LocationItemList key={index} onClick={setLocationOnClick}>
                  <PinIconDiv>
              ...
              >
                ...
                >
                  ...
                  <Container style={{ marginRight: "auto" }}>
                    <BedsAndType>
                      {item.type}
                      {item.beds === null ? null : ` . ${item.beds} beds`}
                    </BedsAndType>
                  </Container>
                  <Container
                    style={{
